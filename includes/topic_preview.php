...
class phpbb_topic_preview
{
	...
	*/
	function phpbb_topic_preview()
	{
		...
		if ($this->tp_avatars) {
			$this->tp_sql_select .= ', fpu.user_colour AS first_user_colour, fpu.user_posts' . (($this->tp_last_post) ? ', lpu.user_colour AS last_user_colour' : '');
		}
...
$no_avatar = ''; // PBWoW 2 MOD for topic-preview mod
			$first_post_avatar = (!empty($row['first_user_avatar'])) ? get_user_avatar($row['first_user_avatar'], $row['first_user_avatar_type'], 64, 64) : $no_avatar;
			$last_post_avatar  = (!empty($row['last_user_avatar'])) ? get_user_avatar($row['last_user_avatar'], $row['last_user_avatar_type'], 64, 64) : $no_avatar;
		}
