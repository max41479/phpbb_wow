	...
	*/
	...
	while ($row = $db->sql_fetchrow($result))
	{
		...
		if ($config['load_cpf_viewtopic'])
		{
			...
			$cheat_cache[$topic_id] = array(
				'tfp' => $row['topic_poster'],
				'tlp' => $row['topic_last_poster_id'],
			);
		}
		// END PBWoW 2 MOD for topic-preview mod
	...
	if ($config['load_cpf_viewtopic'])
	{
		...
		if (function_exists('modify_topic_preview') && isset($profile_fields_cache)) {
			modify_topic_preview(false, $tpl_loopname, $profile_fields_cache, $topic_preview->tp_avatars, $cheat_cache);
		}
	}
	// END PBWoW 2 MOD for topic-preview mod
